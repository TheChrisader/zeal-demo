...
import { validateRequest } from "@/lib/auth/auth"; // Assuming this path, or adjust to '@lib/auth' if index export
import { uploadImageToS3 } from "@/lib/bucket"; // Assuming this path, adjust if different
import { connectToDatabase } from "@/lib/database";
// import { UserRole } from "@/types/user.type"; // Assuming UserRole enum/type exists here
import {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Authorization: Allow if user role is not 'USER'. Moderator check is skipped as per instruction.
    // This means roles like ADMIN, EDITOR, etc., are allowed.
    if (user.role === "user") {
    ...
    return NextResponse.json({ imageUrl: image_url }, { status: 200 });
  } catch (error) {
