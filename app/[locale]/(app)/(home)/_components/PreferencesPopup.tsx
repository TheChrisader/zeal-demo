...
const PreferencesPopup = () => {
  ...
  const arePreferencesDefault =
    preferences.length === defaultPreferences.length &&
    preferences.every((item, index) => item.name === defaultPreferences[index]);

  useEffect(() => {
    if (user && !arePreferencesDefault) {
