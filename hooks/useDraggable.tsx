...
export function useDraggable(
  ...
  const recoverChildStyle = () => {
    (ref.current.childNodes as NodeListOf<HTMLOptionElement>).forEach(
      (child: HTMLElement, i) => {
        child.style.transform = transformStyleOfChildElements[i] as string; // eslint-disable-line no-param-reassign
        child.style.transition = transitionStyleOfChildElements[i] as string; // eslint-disable-line no-param-reassign
      },
    );
  };

  let rubberBandAnimationTimer: NodeJS.Timeout;
  let keepMovingX: number;
  let keepMovingY: number;

  const callbackMomentum = () => {
    const minimumSpeedToTriggerMomentum = 0.05;

    keepMovingX = window.setInterval(() => {
    ...
    keepMovingY = window.setInterval(() => {
  ...
  const onMouseUp = (e: MouseEvent) => {
    ...
    (ref.current.childNodes as NodeListOf<HTMLOptionElement>).forEach(
      (child: HTMLElement, i) => {
        child.style.cursor = cursorStyleOfChildElements[i] as string; // eslint-disable-line no-param-reassign
      },
    );
  ...
  const onMouseMove = (e: MouseEvent) => {
    ...
    ref.current.style.userSelect = "none"; // eslint-disable-line no-param-reassign
    (ref.current.childNodes as NodeListOf<HTMLOptionElement>).forEach(
      (child: HTMLElement) => {
        child.style.userSelect = "none"; // eslint-disable-line no-param-reassign
      },
    );
