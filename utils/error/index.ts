...
export const sendError = <D>(error: ApiError<D>) => {
  const errorObj = {
    ...error,
...
export const getErrorMessage = <D>(
  apiError: ApiError<D>,
  options?: { locale: "fr" | "en" },
) => {
  return (
    errorMessages[apiError.code || DEFAULT_ERROR]![options?.locale || "en"] ||
    errorMessages[DEFAULT_ERROR]![options?.locale || "en"]
  );
};
